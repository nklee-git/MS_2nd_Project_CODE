...
>
> **⚠️ 데이터 연동 방식 확정 (2026-09-14)**: Razor Pages 프론트는 Dataverse를 직접 호출하지 않습니다 — 최민님이 만드는 별도 **C# Web API 서버**(`FashionAI.Api`)를 거칩니다(최민님 C# 개발 경험 목적). 프론트는 이 API가 주는 JSON만 받아 렌더링. 1차는 기존 `SampleDataService`(CSV 기반)로 화면부터 이식하고, 인터페이스를 분리해두면 2차에 API 클라이언트로 교체만 하면 됩니다. 차트 라이브러리는 서버 렌더링 환경에 맞게 Recharts→(예: Chart.js) 교체 검토 필요 — 시각 스펙은 그대로, 라이브러리만 교체. 상세는 [[ML·RAG·프론트·백엔드 실행설계 (요구사항-설계-구현-테스트)]] 3절 참고.
