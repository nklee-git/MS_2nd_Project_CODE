...
> **2026-09-14 갱신**: Dataverse에 원시 테이블 14개(+데이터마트 7개 예정)가 잘못 적재되며 아키텍처 이해가 어긋난 사건을 계기로 재확인·보강. 원시데이터는 Blob 경유(Dataverse 아님), 대시보드는 **최민님이 만드는 별도 C# Web API 서버**를 거쳐 Dataverse에 접근하는 구조로 확정(최민님이 C# 백엔드 개발 경험을 원해 별도 프로젝트로 분리). 실시간 데이터 생성도 이 API 서버 안에서 처리. 상세 설계는 [[ML·RAG·프론트·백엔드 실행설계 (요구사항-설계-구현-테스트)]] 4절 참고.
...
### 롤별 담당 범위 (2026-09-11 4인 체제로 갱신 — 6개 기능영역)
> 롤을 잘게 나누는 대신 "진짜 테크팀처럼" 책임영역 단위로 묶는 방향. 2026-09-10엔 박형준 최종 미합류로 최민·임현제·나경 3인 체제였으나, **2026-09-11 박형준이 합류해 프론트 개발에 착수**하며 4인 체제로 갱신 — 나경은 RAG·자동화 + 대시보드·프론트(박형준과 공동)를 겸임.

| 기능 영역 | 담당 컴포넌트 | 담당자 | Week 0에 먼저 끝나야 하는 이유 |
| --- | --- | --- | --- |
| 데이터 파이프라인 | Blob 업로드 + Azure Data Factory — Databricks 노트북 오케스트레이션 | 최민 | ML이 학습할 데이터셋이 없으면 다음 단계 자체가 시작 안 됨 |
| Azure 인프라 | Dataverse 테이블 스키마(`ReorderRecommendation`+SKU마스터) + Security Role(승인자 MD + 조회자 유관부서 다중, 2026-09-11 확장) | 최민 | **가장 먼저 확정돼야 함** — 나머지가 전부 이 테이블을 보고 작업 |
| **백엔드 API (신규, 2026-09-14)** | **C# Web API 서버**(`FashionAI.Api`) — 대시보드↔Dataverse 사이 유일한 통로, 실시간 데이터 생성도 여기서 처리 | 최민 | 최민님이 C# 백엔드 개발 경험을 원해 별도 프로젝트로 분리. 프론트가 이 API의 계약(엔드포인트 4개)에 의존하므로 최민의 데이터·인프라 작업과 함께 조기 확정 필요 |
...
| 대시보드·프론트 | ASP.NET Core MVC/Razor Pages 대시보드 2뷰, (Should) Power BI | 나경·박형준(2026-09-11 합류, 프론트 개발 착수) | 같은 이유로 Dataverse 스키마에 의존 — 파이프라인의 마지막 소비자 |
