> 관련 문서: [[24. 기능명세서 v1]] · [[26. 대시보드 기술명세 (프론트)]] · [[28. Dataverse 가이드]] · [[29. 파이프라인 아키텍처 재정렬 (초안)]] · [[22. Cost Plan]] · [[23. Alert & Trigger Rules]]
> **상태**: 팀장 확정안. [[29. 파이프라인 아키텍처 재정렬 (초안)]]에서 정한 블로킹 결정(직접 호출 / SKU 마스터 분리 / IHostedService / risk_score 공식 통일)을 전제로, ML·RAG·프론트·백엔드 4개 영역을 요구사항분석→설계→구현→테스트 순서로 확정한다. 각 영역 담당자는 이 문서 기준으로 진행하고, 다른 방향이 필요하면 구두가 아니라 이 문서 개정으로 처리한다.
> **작성 원칙**: 새 기술/새 라이브러리를 되도록 도입하지 않는다 — 이미 존재하는 산출물(`generate_v4.py` 공식, React 목업의 화면/컬러 결정, 23번 risk_score 공식)을 그대로 재사용하는 쪽으로 설계한다. 2.5주 PoC 규모에 맞지 않는 과설계는 배제한다.
...
| 출력 | SKU별 `predicted_demand`/`recommended_qty`/`risk_score` → Dataverse `ReorderRecommendation` upsert (원시데이터는 올리지 않음, [[29. 파이프라인 아키텍처 재정렬 (초안)]] 1절) |
| 처리 주기 | 짧은 주기 micro-batch(5~10분) — 진짜 상시 스트리밍 아님([[22. Cost Plan]]) |
| 정확도 검증 | 9월 held-out(8/21~9/20)과 대조, **9/23 체크포인트**에서 1차 확인 |
| risk_score 공식 | [[23. Alert & Trigger Rules]] 확정 공식(재고소진임박도 × 인기도가중치, **고정 가중치**)으로 통일 — 현재 대시보드 구현(랜덤 구간)은 폐기 |
...
- 예측근거 자연어 설명(ReAct 축소판, [[21. Appservice Core model]] 7절)은 **코어 루프(위 5단계) 배포·검증 완료 후에만** 착수 — Tier1 스트레치, 순서 바뀌면 안 됨
...
- 클러스터: 소형(1~2 노드) + **autotermination 15~30분 필수**([[22. Cost Plan]]), 작업 끝나면 종료 상태 직접 확인
...
### 2-1. 요구사항분석
> 실제 "RAG 챗봇"은 이번 스코프가 아니다([[21. Appservice Core model]] 0절에 이미 확정) — "RAG·자동화" 롤의 실제 작업은 **Power Automate 승인 워크플로우**다. 이름에 낚이지 않는다.
...
> **원칙 충돌 주의**: [[21. Appservice Core model]] 0-1절·7절은 "완전 자동화가 아니라 의사결정 지원(Human-in-the-loop)"을 명시적 설계 원칙으로 못박아뒀다(과신/over-reliance 방지 근거까지 인용됨). 따라서 이 기능은 **완전자동화가 아니라 "SLA 타임아웃 + 저위험 건만 안전 기본값 처리 + 전부 기록에 남겨 사람이 뒤집을 수 있게"**로 스코프를 좁힌다. "담당자 없으면 AI가 다 결정한다"는 버전은 만들지 않는다.
...
**설계**
- Power Automate Flow의 "Wait for a response"에 SLA 타임아웃 추가(예: 근무시간 기준 4시간 — 정확한 값은 일요일 확정)
...
**일정 배치**: 1절 ML 코어 루프 + 2-1~2-4 코어 승인 플로우가 9/23 체크포인트를 통과한 **이후에만** 착수 — 7절 GenAI 스트레치와 같은 순서 원칙(코어 먼저, 스트레치는 그 다음). SLA 시간·임계값 구체적 수치는 일요일 미팅에서 확정.

### 2-6. 위험 알람을 앱까지 도달시키기 — Teams 긴급(Urgent) 우선순위 (신규)

> "어플이랑 연동"을 **별도 커스텀 모바일 앱 신규 개발**로 해석하면 비추천이다: 팀 전원이 이미 Teams 모바일 앱을 쓰고 있어 Adaptive Card 알림이 원래 모바일에도 가고, 새 앱(네이티브/PWA)+푸시 인프라(디바이스 토큰 등록 등)를 지금 만드는 건 [[29. 파이프라인 아키텍처 재정렬 (초안)]] 7절 "기술스택 고정" 원칙과 충돌하며 무료 티어 범위도 벗어난다. 대신 **Teams 자체의 긴급 알림 기능**으로 같은 목적(놓치지 않고 폰까지 알림이 감)을 달성한다.
...
- 긴급임계값은 2-5절 `auto_decidable`(저위험=자동승인 후보)과 반대 개념 — 고위험 쪽 별도 임계값으로 정의(예: risk_score 상위 20%, 정확한 값은 일요일 확정)
...
### 3-1. 요구사항분석
- 화면은 **2개만**([[24. 기능명세서 v1]] 3절 확정): 승인이력 뷰, 예측대조 뷰 — 화면을 늘리지 않고 이 2개를 깊게 만든다
- **(2026-09-13 변경)** 데이터 출처는 Dataverse가 아니라 **최민님의 C# Web API 서버**([[29. 파이프라인 아키텍처 재정렬 (초안)]] 3-①) — 프론트는 Dataverse 인증·OData를 전혀 모른다. API가 주는 JSON만 받아서 렌더링
- 기존 React 목업(v0.5.0)의 **화면 구조·컬러 팔레트·컴포넌트 분해는 그대로 재사용** — 디자인을 새로 정하지 않는다(6색 팔레트, Pretendard, risk_score 앰버 램프 등 [[README|70.FRONT/README.md]] "디자인 결정 사항" 그대로)
...
- 차트는 기존 Recharts 대신 서버 렌더링 환경에 맞는 라이브러리로 교체 필요(예: Chart.js) — 시각적 스펙(라인차트/바차트 구성)은 26번 문서 그대로, 라이브러리만 교체
...
> **2026-09-13 변경**: [[29. 파이프라인 아키텍처 재정렬 (초안)]] 3-① 결정 변경 — 최민님이 C# 백엔드 개발을 직접 해보고 싶다고 확인되어, "임베디드 서비스"가 아니라 **독립된 ASP.NET Core Web API 프로젝트**로 분리한다. 대신 학습 목적과 일정 리스크를 같이 관리하기 위해 **엔드포인트를 아래 4개로 제한**한다 — 늘리고 싶으면 이 문서를 먼저 고치고 시작할 것.
...
| 실시간 데이터 생성 | 신규 주문/재고이동/반품/재발주추천 이벤트를 tick마다 생성해 Dataverse에 반영 ([[작업지시서 (최민 — 데이터·인프라)]] 6-1절 우선순위 표 그대로) — 이 API 서버 안에서 처리(3-③) |
...
- 시뮬레이션 데이터임을 UI에 표시할지는 [[작업지시서 (최민 — 데이터·인프라)]] 6-5절 결정에 따름

---

## 5. 통합 일정 & Definition of Done
...
이 문서와 [[29. 파이프라인 아키텍처 재정렬 (초안)]]이 서로 다른 내용을 담고 있으면 29번(아키텍처/인프라 배치)이 우선, 이 문서(역할별 구현설계)는 그 위에서 실행 세부사항을 정한다.
