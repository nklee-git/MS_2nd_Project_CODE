> 관련 문서: [[21. Appservice Core model]] 6절 · [[24. 기능명세서 v1]] · [[26. 대시보드 기술명세 (프론트)]] · [[28. Dataverse 가이드]] · [[22. Cost Plan]] · [[23. Alert & Trigger Rules]] · [[작업지시서 (최민 — 데이터·인프라)]]
...
| 출력 | SKU별 `predicted_demand`/`recommended_qty`/`risk_score` → Dataverse `ReorderRecommendation` upsert (원시데이터는 올리지 않음, [[21. Appservice Core model]] 6절) |
...
> "어플이랑 연동"을 **별도 커스텀 모바일 앱 신규 개발**로 해석하면 비추천이다: 팀 전원이 이미 Teams 모바일 앱을 쓰고 있어 Adaptive Card 알림이 원래 모바일에도 가고, 새 앱(네이티브/PWA)+푸시 인프라(디바이스 토큰 등록 등)를 지금 만드는 건 [[12. KickOFF]] "기술스택 고정 원칙"(확정 후 변경은 문서 개정으로만)과 충돌하며 무료 티어 범위도 벗어난다. 대신 **Teams 자체의 긴급 알림 기능**으로 같은 목적(놓치지 않고 폰까지 알림이 감)을 달성한다.
...
- **(2026-09-13 변경)** 데이터 출처는 Dataverse가 아니라 **최민님의 C# Web API 서버**([[작업지시서 (최민 — 데이터·인프라)]] 1-2절, [[28. Dataverse 가이드]] 3-5절) — 프론트는 Dataverse 인증·OData를 전혀 모른다. API가 주는 JSON만 받아서 렌더링
...
> **2026-09-13 확정**: 최민님이 C# 백엔드 개발을 직접 해보고 싶다고 확인되어, "임베디드 서비스"가 아니라 **독립된 ASP.NET Core Web API 프로젝트**로 분리한다([[작업지시서 (최민 — 데이터·인프라)]] 1-2절 동일 반영). 대신 학습 목적과 일정 리스크를 같이 관리하기 위해 **엔드포인트를 아래 4개로 제한**한다 — 늘리고 싶으면 이 문서를 먼저 고치고 시작할 것.
...
인프라 배치(어디에 뭘 두는가)는 [[21. Appservice Core model]]·[[22. Cost Plan]]·[[28. Dataverse 가이드]]·[[작업지시서 (최민 — 데이터·인프라)]]가 우선하고, 이 문서(역할별 구현설계)는 그 위에서 실행 세부사항을 정한다. 서로 어긋나 보이면 저 문서들 쪽이 최신 인프라 결정이다.
