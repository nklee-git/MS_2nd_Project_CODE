...
- [[04.MS-DataSchool/20.PROJECT/22.MS_2nd_Project/00.WBS/03. WBS 상세 (기술 레이어별 일정표)]] — 데이터·인프라 작업 목록
...
관련 문서 반영: [[16. PPT plan]] · [[21. Appservice Core model]] · [[23. Alert & Trigger Rules]] · [[71. Entity Definitions & Data Dictionary]] · [[72. Entity Relationship Diagram]] · [[42. Channel & Settlement Terms]] · [[14. 1차 프로젝트 대비 차별화 아이디어]] · [[28. Dataverse 가이드]]

## 📌 2026-09-14 전달사항 — 위 14개 CSV/Dataverse 적재 건 최종 정리 + 신규 작업

위 9/11 항목에서 만든 `dataverse_import/` 14개 CSV를 Dataverse에 올리려던 작업은 아키텍처와 어긋나 있었음이 확인됨(최민님 책임 아님, 사전 지시가 문서와 안 맞았던 것 — [[22. Cost Plan]] "Dataverse에 원시데이터를 통째로 올리면 안 되는 이유" 참고). 정리 방향:

- 14개 CSV는 버리지 않고 **Blob Storage 업로드용**으로 그대로 재사용(목적지만 변경)
- "+7개 예정" 데이터마트(`stg_*`/`dim_*`/`fact_*`)는 [[21. Appservice Core model]] 8-2절 백로그 항목과 정확히 일치 — 이미 Year 2/3로 제외 결정돼 있던 것, 작업 중단
- Dataverse에 이미 만든 원시 테이블 14개는 삭제 예정(9/14 회의 확정)

**신규 작업(2026-09-14 확정)**: 최민님이 C# 백엔드 개발을 해보고 싶다고 하셔서, 대시보드↔Dataverse 사이에 **별도 C# Web API 서버(`FashionAI.Api`)**를 최민님이 신규 구축하기로 함 — [[작업지시서 (최민 — 데이터·인프라)]] 1-2절, [[ML·RAG·프론트·백엔드 실행설계 (요구사항-설계-구현-테스트)]] 4절 참고.
