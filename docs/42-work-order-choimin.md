...
> 참고 문서: [22-feature-spec](./22-feature-spec.md) 1·2절 · *2-schedule (기획 볼트 전용, 여기에 없음)* · *21-system-model (기획 볼트 전용, 여기에 없음)* · *61-entity-dictionary (기획 볼트 전용, 여기에 없음)* · [24-alert-rules](./24-alert-rules.md) · *53-virtual-team-profiles (기획 볼트 전용, 여기에 없음)* · [5-dataverse-guide](./5-dataverse-guide.md)
...
| 백엔드 아키텍처        | 별도 **C# Web API 서버**(`FashionAI.Api`) — 최민님이 C# 개발 경험을 원해서 "직접 호출" 대신 이 방향. 엔드포인트 4개 제한. 상세: [25-execution-design](./25-execution-design.md) 4절 |
| 테크리드            | 최민 — 이견 조율 최종 결정권만, 책임은 팀 전체 분담(*1-kickoff-brief (기획 볼트 전용, 여기에 없음)* 8절)                                                                 |
| `risk_score` 공식 | [24-alert-rules](./24-alert-rules.md) 공식(고정 가중치)으로 통일 — 대시보드의 랜덤 구간 구현은 폐기                                                                 |
...
| `auto_decidable` (Boolean, 신규) | ✅ 추가 — ML이 계산, 저위험 자동승인 후보 여부. [25-execution-design](./25-execution-design.md) 2-5절   |
...
- [ ] Dataverse에 이미 만든 원시 테이블 14개 삭제, 데이터마트 7개 작업 중단(*21-system-model (기획 볼트 전용, 여기에 없음)* 부록 B, Year2/3 백로그)
...
| 5 | 협업허브 알림(`ALERT_LOG`, 백로그 엔티티) | 4번과 동시 | *53-virtual-team-profiles (기획 볼트 전용, 여기에 없음)* 3-4절 참고 |

**4-2. 재사용할 계산 공식** (새로 만들지 않음, `generate_v4.py`/*61-entity-dictionary (기획 볼트 전용, 여기에 없음)* 그대로)
...
- 트렌드캡슐(`line_type=TREND`)은 자동재발주 트리거 제외(*24-sku-growth-roadmap (기획 볼트 전용, 여기에 없음)*)
...
## 결론 — 아직 열린 질문
9/14 회의에서 확정할 것:
- [ ] `ALERT_LOG` 발생 시 협업허브에 즉시 반영할지 (*53-virtual-team-profiles (기획 볼트 전용, 여기에 없음)* 3-4절 그대로 vs 재발주 추천까지만)
- [ ] 시뮬레이션 데이터임을 UI에 "실시간 시뮬레이션" 뱃지로 표시할지

결정되는 대로 [22-feature-spec](./22-feature-spec.md) 1-1절과 *61-entity-dictionary (기획 볼트 전용, 여기에 없음)*에 반영한다.
