> 📌 **이 문서는 옵시디언 PM 볼트의 스냅샷입니다** (동기화: 2026-09-14). 실시간 최신본은 볼트 쪽이 원본이며, 이 사본은 팀원이 코드 작업 중 참고용으로만 주기적으로 갱신됩니다. 변경이 필요하면 볼트에서 먼저 고치고 PM(나경)에게 알려주세요.

> 관련 문서: *21-system-model (기획 볼트 전용, 여기에 없음)* 2절 · [22-feature-spec](./22-feature-spec.md) · *72-dashboard-spec (기획 볼트 전용, 여기에 없음)* · [5-dataverse-guide](./5-dataverse-guide.md) · *23-cost-plan (기획 볼트 전용, 여기에 없음)* · [24-alert-rules](./24-alert-rules.md) · [42-work-order-choimin](./42-work-order-choimin.md)
...
> **작성 원칙**: 새 기술/새 라이브러리를 되도록 도입하지 않는다 — 이미 존재하는 산출물(`generate_v4.py` 공식, React 목업의 화면/컬러 결정, [24-alert-rules](./24-alert-rules.md) risk_score 공식)을 그대로 재사용하는 쪽으로 설계한다. 2.5주 PoC 규모에 맞지 않는 과설계는 배제한다.

## 서론
ML(임현제)·RAG(나경)·프론트(나경·박형준)·백엔드(최민, 신규 C# API) 4개 영역 각각을 **요구사항분석→설계→구현→테스트** 순서로 정리했다. 코어(1절, 2-1~2-4절, 3절, 4절)가 먼저이고, 스트레치(2-5·2-6절)는 **9/23 체크포인트를 통과한 뒤에만** 착수 — 순서를 바꾸면 코어도 못 끝낸 채 부가기능에 시간을 쓰게 된다.
...
| 출력 | SKU별 `predicted_demand`/`recommended_qty`/`risk_score` → Dataverse `ReorderRecommendation` upsert (원시데이터는 올리지 않음, *21-system-model (기획 볼트 전용, 여기에 없음)* 2절) |
| 처리 주기 | 짧은 주기 micro-batch(5~10분) — 진짜 상시 스트리밍 아님(*23-cost-plan (기획 볼트 전용, 여기에 없음)*) |
| 정확도 검증 | 9월 held-out(8/21~9/20)과 대조, **9/23 체크포인트**에서 1차 확인 |
| risk_score 공식 | [24-alert-rules](./24-alert-rules.md) 확정 공식(재고소진임박도 × 인기도가중치, **고정 가중치**)으로 통일 — 현재 대시보드 구현(랜덤 구간)은 폐기 |
...
- 예측근거 자연어 설명(ReAct 축소판, *21-system-model (기획 볼트 전용, 여기에 없음)* 부록 A)은 **코어 루프(위 5단계) 배포·검증 완료 후에만** 착수 — Tier1 스트레치, 순서 바뀌면 안 됨
...
- 클러스터: 소형(1~2 노드) + **autotermination 15~30분 필수**(*23-cost-plan (기획 볼트 전용, 여기에 없음)*), 작업 끝나면 종료 상태 직접 확인
...
### 2-1. 요구사항분석
> 실제 "RAG 챗봇"은 이번 스코프가 아니다(*21-system-model (기획 볼트 전용, 여기에 없음)* 서론에 이미 확정) — "RAG·자동화" 롤의 실제 작업은 **Power Automate 승인 워크플로우**다. 이름에 낚이지 않는다.
...
> HITL 원칙(*21-system-model (기획 볼트 전용, 여기에 없음)* 서론, 과신 방지 근거 포함) 준수 위해 스코프를 좁힌다: **완전자동화 아님 — SLA 타임아웃 + 저위험 건만 안전 기본값 처리 + 전부 기록에 남겨 사람이 뒤집을 수 있게.** "담당자 없으면 AI가 다 결정한다"는 버전은 만들지 않는다.
...
> "어플이랑 연동"을 **별도 커스텀 모바일 앱 신규 개발**로 해석하면 비추천이다: 팀 전원이 이미 Teams 모바일 앱을 쓰고 있어 Adaptive Card 알림이 원래 모바일에도 가고, 새 앱(네이티브/PWA)+푸시 인프라(디바이스 토큰 등록 등)를 지금 만드는 건 *1-kickoff-brief (기획 볼트 전용, 여기에 없음)* "기술스택 고정 원칙"(확정 후 변경은 문서 개정으로만)과 충돌하며 무료 티어 범위도 벗어난다. 대신 **Teams 자체의 긴급 알림 기능**으로 같은 목적(놓치지 않고 폰까지 알림이 감)을 달성한다.
...
## 3. 프론트 — ASP.NET Core MVC/Razor Pages (담당: 나경·박형준)

### 3-1. 요구사항분석
- 화면은 **2개만**([22-feature-spec](./22-feature-spec.md) 3절 확정): 승인이력 뷰, 예측대조 뷰 — 화면을 늘리지 않고 이 2개를 깊게 만든다
- **(2026-09-13 변경)** 데이터 출처는 Dataverse가 아니라 **최민님의 C# Web API 서버**([42-work-order-choimin](./42-work-order-choimin.md) 1-2절, [5-dataverse-guide](./5-dataverse-guide.md) 3-5절) — 프론트는 Dataverse 인증·OData를 전혀 모른다. API가 주는 JSON만 받아서 렌더링
- 기존 React 목업(v0.5.0)의 **화면 구조·컬러 팔레트·컴포넌트 분해는 그대로 재사용** — 디자인을 새로 정하지 않는다(6색 팔레트, Pretendard, risk_score 앰버 램프 등 *70-frontend/readme.md (기획 볼트 전용, 여기에 없음)* "디자인 결정 사항" 그대로)
...
- 차트는 기존 Recharts 대신 서버 렌더링 환경에 맞는 라이브러리로 교체 필요(예: Chart.js) — 시각적 스펙(라인차트/바차트 구성)은 *72-dashboard-spec (기획 볼트 전용, 여기에 없음)* 그대로, 라이브러리만 교체
...
> **2026-09-13 확정**: 최민님이 C# 백엔드 개발을 직접 해보고 싶다고 확인되어, "임베디드 서비스"가 아니라 **독립된 ASP.NET Core Web API 프로젝트**로 분리한다([42-work-order-choimin](./42-work-order-choimin.md) 1-2절 동일 반영). 대신 학습 목적과 일정 리스크를 같이 관리하기 위해 **엔드포인트를 아래 4개로 제한**한다 — 늘리고 싶으면 이 문서를 먼저 고치고 시작할 것.
...
| 실시간 데이터 생성 | 신규 주문/재고이동/반품/재발주추천 이벤트를 tick마다 생성해 Dataverse에 반영 ([42-work-order-choimin](./42-work-order-choimin.md) 4-1절 우선순위 표 그대로) — 이 API 서버 안에서 처리(3-③) |
...
- Swagger/OpenAPI 자동 노출 — 학습용으로도, 나경/박형준과의 계약 문서로도 겸용
...
- 시뮬레이션 데이터임을 UI에 표시할지는 [42-work-order-choimin](./42-work-order-choimin.md) 결론 결정에 따름
...
인프라 배치(어디에 뭘 두는가)는 *21-system-model (기획 볼트 전용, 여기에 없음)*·*23-cost-plan (기획 볼트 전용, 여기에 없음)*·[5-dataverse-guide](./5-dataverse-guide.md)·[42-work-order-choimin](./42-work-order-choimin.md)가 우선하고, 이 문서(역할별 구현설계)는 그 위에서 실행 세부사항을 정한다. 서로 어긋나 보이면 저 문서들 쪽이 최신 인프라 결정이다.
